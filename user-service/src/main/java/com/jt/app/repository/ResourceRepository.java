...
import com.jt.app.model.Resource;
...
public interface ResourceRepository extends JpaRepository<Resource, Integer> {
    ...
    /*@Query(value = "SELECT c.*\n" +
            ...
            "       SELECT *\n" +
            "       FROM cart_event\n" +
            "       WHERE Resource_id = ?1 AND (cart_event_type = 3 OR cart_event_type = 2)\n" +
            "       ORDER BY cart_event.created_at DESC\n" +
            "       LIMIT 1\n" +
            "     ) t\n" +
            "  RIGHT JOIN cart_event c ON c.Resource_id = t.Resource_id\n" +
    ...
    Channel getCartEventStreamByResource(Long ResourceId);*/

    @Modifying
    @Query("update Resource r set r.createTime=?1, r.updateTime=?2 where r.id=?3")
